...
open class MediaLibrarySessionCallback(
    ...
    override fun onConnect(
        ...
        if (BuildConfig.DEBUG) {
            Log.d(
                tag,
                "onConnect pkg=${controller.packageName} auto=${session.isAutomotiveController(controller)} " +
    ...
    override fun onMediaButtonEvent(
        ...
        if (BuildConfig.DEBUG) {
            Log.d(
                tag,
                "onMediaButtonEvent pkg=${controllerInfo.packageName} key=${keyEvent?.keyCode} action=${keyEvent?.action}"
            )
        }
        if (keyEvent == null) {
