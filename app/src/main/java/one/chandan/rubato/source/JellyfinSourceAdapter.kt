...
class JellyfinSourceAdapter(
    serverId: String,
    displayName: String? = null
) : MediaSourceAdapter<ArtistID3, AlbumID3, Child, Playlist> {
    private val resolvedServerId = serverId.trim().ifEmpty { MediaSourceType.JELLYFIN.id }

    override val source: MediaSourceRef =
        MediaSourceRef(resolvedServerId, MediaSourceType.JELLYFIN, displayName)

    override fun mapArtist(item: ArtistID3): MediaArtist? {
        val name = item.name?.trim().orEmpty()
        if (name.isEmpty()) return null
        val id = MediaIdResolver.jellyfin(resolvedServerId, item.id, name)
        return MediaArtist(
            id = id,
            name = name,
    ...
    override fun mapAlbum(item: AlbumID3): MediaAlbum? {
        ...
        val id = MediaIdResolver.jellyfin(resolvedServerId, item.id, title)
        val artistId = item.artistId?.let { MediaIdResolver.jellyfin(resolvedServerId, it, it) }
        return MediaAlbum(
            id = id,
            title = title,
    ...
    override fun mapSong(item: Child): MediaSong? {
        ...
        val id = MediaIdResolver.jellyfin(resolvedServerId, item.id, title)
        val artistId = item.artistId?.let { MediaIdResolver.jellyfin(resolvedServerId, it, it) }
        val albumId = item.albumId?.let { MediaIdResolver.jellyfin(resolvedServerId, it, it) }
        return MediaSong(
            id = id,
            title = title,
            artist = item.artist?.trim(),
    ...
    private fun coverArt(coverArtId: String?): MediaImage? {
        ...
        val resolved = MediaIdResolver.jellyfin(resolvedServerId, id, id)
        return MediaImage(resolved)
    }
}
