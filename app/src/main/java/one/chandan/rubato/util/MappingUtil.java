...
public class MappingUtil {
    ...
    public static MediaItem mapMediaItem(Child media) {
        ...
        Uri artworkUri = buildArtworkUri(media.getCoverArtId(), LocalMusicRepository.isLocalSong(media));
    ...
    private static Uri buildArtworkUri(String coverArtId, boolean isLocal) {
        if (coverArtId == null) return Uri.EMPTY;
        if (isLocal) {
            return Uri.parse(coverArtId);
        }
        return Uri.parse(CustomGlideRequest.createUrl(coverArtId, Preferences.getImageSize()));
    }
}
